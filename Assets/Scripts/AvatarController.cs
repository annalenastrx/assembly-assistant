...
/// <summary>
/// Class <c>AvatarController</c> handles the different conditions and coordinates
/// between the different states.
/// </summary>
[RequireComponent(typeof(AudioSource))]
public class AvatarController : MonoBehaviour
{
    //Inspector input for condition order
    ...
    void Update()
    {
        if(timeCounting) time += Time.deltaTime;

        if ((Input.GetKeyDown("space") || remoteKeyDown == "space")  && building)
        {
            if (nextCond)
            {
                StartNextCondition();
        ...
        else if(remoteKeyDown == "mouse" && pointing)
        {
            if (nextCond)
            {
                StartNextCondition();
            ...
            {
                targetPointing.position = remoteTargetPos;
        ...
        else if(Input.GetKeyDown(KeyCode.LeftArrow) || remoteKeyDown == "left")
        {
            voiceInstructionCounter--;
            PlayAudioClip(false);
        }
        else if(Input.GetKeyDown(KeyCode.RightArrow) || remoteKeyDown == "right")
        {
            PlayAudioClip(true);
        }
        else if (Input.GetKeyDown(KeyCode.DownArrow)|| remoteKeyDown == "down")
        {
    ...
    public void StartNextCondition(){
        ...
        {
            textManager.WriteCondition(conditions[0,conditionCounter-1],conditions[1,conditionCounter-1]);
            textManager.ExportTime(time);
        }
    ...
    private void PlayAudioClip(bool changeModel)
    {
        ...
        if (voiceInstructionCounter == 1) timeCounting = true;
    ...
    /// <summary>
    /// This method sets the remote key if it was pressed.
    /// </summary>
    /// <param name="key">The key that was pressed remote.</param>
    public void SetRemoteKeyDown(string key)
    {
        remoteKeyDown = key;
    }

    /// <summary>
    /// This method sets the pointing target position if it was pressed remote.
    /// </summary>
    /// <param name="remoteTargetPos">The remote target position.</param>
    public void SetTargetPos(Vector3 remoteTargetPos)
    {
        this.remoteTargetPos = remoteTargetPos;
    }
